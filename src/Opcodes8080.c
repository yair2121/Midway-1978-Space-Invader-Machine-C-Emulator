...
static void handle_mov(Cpu8080* cpu, State8080* state, uint8_t* opcode) { // MOV TARGET, SOURCE
...
static void handle_nop(Cpu8080* cpu, State8080* state, uint8_t* opcode) {}

SINGLE_VALUE_SHOULD_HANDLE(HLT);
static void handle_hlt(Cpu8080* cpu, State8080* state, uint8_t* opcode) {}
...
void handle_add(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	uint8_t value_to_add = get_single_target_value(state, opcode, ADD, ADD_IMMEDIATE);
	op_ADD(state, value_to_add);
}


RANGE_AND_SINGLE_VALUE_SHOULD_HANDLE(ADC, ADC_END_RANGE, ADC_IMMEDIATE); // ADC immediate value to A register
void handle_adc(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	uint16_t value_to_add = get_single_target_value(state, opcode, ADC, ADC_IMMEDIATE);
	op_ADD(state, value_to_add + state->cc[CARRY]);
}
...
static void handle_sub(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	uint16_t value_to_sub = get_single_target_value(state, opcode, SUB, SUB_IMMEDIATE);
	op_sub(state, value_to_sub);
}

RANGE_AND_SINGLE_VALUE_SHOULD_HANDLE(SBB, SBB_END_RANGE, SBB_IMMEDIATE); // SUB immediate value from A register
static void handle_sbb(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	uint16_t value_to_sbb = get_single_target_value(state, opcode, SBB, SBB_IMMEDIATE);
	op_sub(state, value_to_sbb + state->cc[CARRY]);
}


RANGE_AND_SINGLE_VALUE_SHOULD_HANDLE(ANA, ANA_END_RANGE, ANA_IMMEDIATE); // ANA immediate value with A register
static void handle_ana(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	uint8_t targetValue = get_single_target_value(state, opcode, ANA, ANA_IMMEDIATE);
	state->general_register[A] = state->general_register[A] & targetValue;
	set_flagsZSP(state, state->general_register[A]);
...
static void handle_xra(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	uint8_t targetValue = get_single_target_value(state, opcode, XRA, XRA_IMMEDIATE);
	op_xra(state, targetValue);
}
...
static void handle_ora(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	uint8_t targetValue = get_single_target_value(state, opcode, ORA, ORA_IMMEDIATE);
	op_ora(state, targetValue);
}
...
static void handle_cmp(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	uint16_t value_to_cmp = get_single_target_value(state, opcode, CMP, CMP_IMMEDIATE);
	op_cmp(state, value_to_cmp);
}

DISCRETE_VALUE_SHOULD_HANDLE(LXI, LXI_B, LXI_D, LXI_H, LXI_SP);
handle_lxi(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	...
	if (is_pair(special_register_to_set)) {
		set_register_pair(state, special_register_to_set, value_to_set);
	}
	else {
		state->sp = value_to_set;
	}
}

DISCRETE_VALUE_SHOULD_HANDLE(STAX, STAX_B, STAX_D);
static void handle_stax(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	SPECIAL_REGISTER special_register = *opcode / 0x10; // opcodes layout distance is 0x10.
	write_to_register_pair_address(state, BC, state->general_register[A]);
}
...
static void op_ret(State8080* state) {
	uint8_t low = pop_byte(state);
	uint8_t high = pop_byte(state);
	state->pc = bytes_to_word(low, high);
}

static void op_jmp(State8080* state, uint8_t low, uint8_t high) { state->pc = bytes_to_word(low, high); }
...
bool should_do_conditional_op(State8080* state, OPCODE opcode, OPCODE base_opcode) {
	if (opcode == JMP || opcode == CALL || opcode == RET) { // No condition case
		return true;
	}
...
static void handle_sta(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
...
static void handle_daa(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	op_daa(state);
}

void op_in(Cpu8080* cpu, State8080* state, uint8_t port) {
...
static void handle_in(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	op_in(cpu, state, opcode[1]);
}

void op_out(Cpu8080* cpu, State8080* state, uint8_t port) {
	cpu->outTask.writePort(port, state->general_register[A], cpu->outTask.context);
	state->pc++;
}
SINGLE_VALUE_SHOULD_HANDLE(OUT);
static void handle_out(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	op_out(cpu, state, opcode[1]);
}

DISCRETE_VALUE_SHOULD_HANDLE(DI_EI, DI, EI); // Disable and Enable Interrupts
static void handle_di_ei(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	state->interrupt_enable = *opcode == EI; // Disable interrupts
}

SINGLE_VALUE_SHOULD_HANDLE(PCHL); // Load register pair immediate value
static void handle_pchl(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	state->pc = get_register_pair(state, HL);
}


void op_mvi(State8080* state, GENERAL_REGISTER source_register, uint8_t target_value) {
...
static void handle_mvi(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
...
static void handle_dad(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
...
static void handle_stc(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	state->cc[CARRY] = true;
}

SINGLE_VALUE_SHOULD_HANDLE(CMC); // Load register pair immediate value
static void handle_cmc(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
...
static void handle_dcx(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	SPECIAL_REGISTER special_register = *opcode / 0x10; // opcodes layout distance is 0x10.
	op_dcx(state, special_register);
}


void op_ldax(State8080* state, SPECIAL_REGISTER register_pair) {
...
static void handle_push(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
...
void op_pop_psw(State8080* state) {
...
static void handle_pop(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
...
static void handle_rst(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	uint8_t rst_number = (*opcode - RST0) / 8; // RST opcodes are spaced by 8
	RST_opcode(state, rst_number);
}


void op_lda(State8080* state, uint16_t address) {
	state->general_register[A] = read_from_memory(state, address);
	state->pc += 2;
}

SINGLE_VALUE_SHOULD_HANDLE(LDA);
static void handle_lda(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	uint16_t address = bytes_to_word(opcode[1], opcode[2]);
	op_lda(state, address);
}

void op_rlc(State8080* state) {
...
static void handle_rlc(Cpu8080* cpu, State8080* state, uint8_t* opcode) { op_rlc(state); }
static void handle_rrc(Cpu8080* cpu, State8080* state, uint8_t* opcode) { op_rrc(state); }
static void handle_ral(Cpu8080* cpu, State8080* state, uint8_t* opcode) { op_ral(state); }
static void handle_rar(Cpu8080* cpu, State8080* state, uint8_t* opcode) { op_rar(state); }


void op_sphl(State8080* state) { state->sp = get_register_pair(state, HL); }
SINGLE_VALUE_SHOULD_HANDLE(SPHL);
static void handle_sphl(Cpu8080* cpu, State8080* state, uint8_t* opcode) { op_sphl(state); }

void op_xchg(State8080* state) {
...
static void handle_xchg(Cpu8080* cpu, State8080* state, uint8_t* opcode) { op_xchg(state); }

void op_xthl(State8080* state) {
...
static void handle_xthl(Cpu8080* cpu, State8080* state, uint8_t* opcode) { op_xthl(state); }


void op_cma(State8080* state) { state->general_register[A] = ~state->general_register[A]; }
SINGLE_VALUE_SHOULD_HANDLE(CMA);
static void handle_cma(Cpu8080* cpu, State8080* state, uint8_t* opcode) { op_cma(state); }

void op_shld(State8080* state, uint16_t address) {
...
static void handle_shld(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	uint16_t address = bytes_to_word(opcode[1], opcode[2]);
	op_shld(state, address);
	state->pc += 2;
}

void op_lhld(State8080* state, uint16_t address) {
...
static void handle_lhld(Cpu8080* cpu, State8080* state, uint8_t* opcode) {
	uint16_t address = bytes_to_word(opcode[1], opcode[2]);
